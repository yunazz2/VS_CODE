...
$(function() {
    ...
    */
    ...
    $('#animate').on('click', function() {
        ...
        if(sw) {
            ...
            $('#box4').animate({
                'opacity'   : '0.3',
                'width'     : '600px',
                // 'background-color'  : 'pink' => 적용 불가 - 맨 위에 참고
            }, 3000)
            ...
            $('#box4').stop().animate({
                // stop을 안 쓰면 광클 한 만큼 애니메이션이 실행된다. 그걸 방지하기위해 쓴다.
                'opacity'   : '1',
                'width'     : '150px',
            }, 3000)
        }
    ...
    /*
        .animate() 메소드에서는 적용할 수 있는 스타일 속성이 제한되어 있다.

        * 원하는 스타일을 지정하여 애니메이션을 적용하는 방법
        1. 이벤트 정의
        2. 적용할 요소에 'active' 등의 이름(원하는 이름)으로 클래스 속성을 추가/삭제
    ...
    */
        ...
        $('#active').on('click', function() {
            if( sw2 ) {
                $('#box5').removeClass('rereset')
                $('#box5').addClass('acactive')
            }
            else {
                $('#box5').removeClass('acactive')
                $('#box5').addClass('rereset')
            }
            sw2 = !sw2
        })
